...
logger = logging.get_logger(__name__)  # __name__为模块名称（如”__main__“）的变量，以生成模块化、层级化的日志
...
# transformers库中的Trainer类成员函数
# 使用时用`from transformers import Trainer`导入，用`trainer = Trainer(...)  trainer.train()`调用
def train(
        self,
        resume_from_checkpoint: Optional[Union[str, bool]] = None,  # Optional[某类型]等价于Union[某类型, None]
        trial: Union["optuna.Trial", Dict[str, Any]] = None,  # 设置一次trial中可以调整的超参数
    ...
    Args:
        ...
        kwargs (`Dict[str, Any]`, *optional*):
            Additional keyword arguments used to hide deprecated arguments  # deprecated arguments指已经被弃用的参数，也就是官方不推荐再使用、可能在将来版本中会被移除的参数。
    """
    if resume_from_checkpoint is False:
        resume_from_checkpoint = None

    # memory metrics - must set up as early as possible
    self._memory_tracker.start()  # 追踪内存使用情况的对象（from .trainer_utils import TrainerMemoryTracker）
    ...
    if self.neftune_noise_alpha is not None:  # NEFTune: Noise Embedding Fine-Tune, 扩大训练集的技术
    ...
    inner_training_loop = find_executable_batch_size(  # 该函数返回一个functools.partial对象，该对象可以以函数的方式调用
    ...
    if args.push_to_hub:
        ...
        finally:  # 无论try中的是否发生异常，最后都要执行finally的部分
...
class partial:
    ...
    __slots__ = "func", "args", "keywords", "__dict__", "__weakref__"  # 用于优化内存，它限制了实例只能有这些属性

    # 创建实例时的构造器，做了以下几件事：
    # 1. 检查 func 是否可调用；
    # 2. 如果 func 已经是 partial，则合并已有的参数（递归支持）；
    # 3. 保存新的函数和参数。
    def __new__(cls, func, /, *args, **keywords):  # __new__是创建对象实例的“构造器”方法，在__init__之前被调用，其中的cls表示当前要创建的类
        if not callable(func):
            raise TypeError("the first argument must be callable")

        # 检查func是否有”func“属性，即检查func这个对象是不是已经是一个partial对象（因为partial对象有一个.func属性，指向原始函数）
        # 在返回partial的find_executable_batch_size()函数中可以发现，该函数是递归调用的，所以此处有拼接参数的操作
        if hasattr(func, "func"):
            args = func.args + args
            keywords = {**func.keywords, **keywords}
            func = func.func

        # super(partial, cls)：表示在类 partial 的继承体系中，找到 cls 对应的父类（这里是 object）。
        # .__new__(cls)：就是调用这个父类的 __new__ 方法，创建一个对象实例。
        # 此句表示用父类的__new__方法创建一个cls类的新对象，并把它赋给self
    ...
    def __call__(self, /, *args, **keywords):  # 调用对象函数时为原始函数补全参数
        keywords = {**self.keywords, **keywords}
        return self.func(*self.args, *args, **keywords)

    @recursive_repr()
    def __repr__(self):  # 	打印友好表示，返回类的可读表示
    ...
    def __reduce__(self):  # 让该对象可以被序列化
...
def find_executable_batch_size(
    ...
    if function is None:
        return functools.partial(
            find_executable_batch_size,  # 递归
            starting_batch_size=starting_batch_size,
            auto_find_batch_size=auto_find_batch_size,
        )

    if auto_find_batch_size:
        # 调用find_executable_batch_size时，通过requires_backends检查是否安装了accelerate库
        ...
        # function为需要动态调整批次大小的训练 / 推理函数
        # starting_batch_size为初始测试的批次大小（例如64），函数会从这个值开始逐步增加或减少，寻找最大可行值。
        return accelerate_find_executable_batch_size(function=function, starting_batch_size=starting_batch_size)

    return functools.partial(function, batch_size=starting_batch_size)


# 实际进行train的函数。find_executable_batch_size()为这个函数寻找batch size
